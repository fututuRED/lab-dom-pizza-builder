...
function renderGlutenFreeCrust() {
  ...
  document.querySelectorAll('.crust-gluten-free').forEach((oneGlutenFree) => {
    if (state.glutenFreeCrust) {
      oneGlutenFree.style.visibility = 'visible';
    } else {
      oneGlutenFree.style.visibility = 'hidden';
    }
  });
}
...
function renderButtons() {
  ...
  buttons.pepperoni.classList.toggle('active', state.pepperoni);
  buttons.mushrooms.classList.toggle('active', state.mushrooms);
  buttons.greenPeppers.classList.toggle('active', state.greenPeppers);
  buttons.whiteSauce.classList.toggle('active', state.whiteSauce);
  buttons.glutenFreeCrust.classList.toggle('active', state.glutenFreeCrust);
}
...
function renderPrice() {
  ...
  for (let ingredient in ingredients) {
    if (state[ingredient]) {
      ...
      pricePanel.innerHTML += `<li>$${ingredients[ingredient].price} ${ingredients[ingredient].name}</li>`;
    }
  }
